...
- 着手前に、**2〜6ステップの短い計画**（調査→実装→検証→共有）を提示する
- 変更前に既存実装を探索し、影響範囲（contracts / api / bot / docs）を簡潔に列挙する
- 要件が曖昧な場合は最大3つまで確認質問をしてから実装する（推測で仕様を増やさない）
- 「ついでの改善」（依頼範囲外のUX追加、設計変更、リファクタ、依存追加）はしない
...
- **差分が200行を超える可能性が高い場合は、着手前に確認**する  
  例：「この指示では変更量が200行を超える可能性がありますが、実行しますか?」
...
- 既存実装を探索し、影響範囲（contracts / api / bot / docs）を列挙する
- 要件が曖昧な場合は最大3つまで確認質問してから実装する（推測で仕様を増やさない）
- API 追加/変更は「contracts → api → bot → docs」の順で行う（契約ファースト）
- 検証は「部分確認 → 最終確認」の順で実行し、結果を報告する
  - 部分確認：`pnpm --filter @yomicord/contracts build` / `pnpm --filter @yomicord/api build` / `pnpm --filter @yomicord/bot build`
  - 最終確認：`pnpm check`
